...
/*
	public static void main(String args[])
	{
		...
		if (OPERATION_PDFEX_BLOCKIFY.equals(operationType))
		{
			...
			try
			{
				pipeline = new CommandLineFitPipeline(inputFolder, true, false, outputFolder, logger,itemsToSkip,endIndex);
			} catch (Exception e)
			{
		...
		} else if (OPERATION_PDFEX_BLOCK_STATISTICS.equals(operationType))
		{
			...
			try
			{
				pipeline = new CommandLineFitPipeline(inputFolder, true, true, outputFolder, logger,itemsToSkip,endIndex);
			} catch (Exception e)
			{
		...
		}else if (OPERATION_PDFEX_BLOCKIFY_CLASSIFY.equals(operationType))
		{
			...
			try
			{
				pipeline = new CommandLineFitPipeline(inputFolder, ruleFileLocation, false, true, outputFolder, logger,itemsToSkip,endIndex);
			} catch (Exception e)
			{
				e.printStackTrace();
				printUsage();
		...
		} else if (OPERATION_FILTERED_LAYOUTAWARE_FULLTEXT_EXTRACTION.equals(operationType))
		{
			...
			try
			{
				...
				pipeline = new CommandLineFitPipeline(inputFolder, ruleFileLocation, false, false, outputFolder, logger,itemsToSkip,endIndex);
			} catch (Exception e)
			{
				e.printStackTrace();
				printUsage();
